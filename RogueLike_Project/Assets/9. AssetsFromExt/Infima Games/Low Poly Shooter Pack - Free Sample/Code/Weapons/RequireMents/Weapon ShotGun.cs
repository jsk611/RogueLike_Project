...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    /// </summary>
    public class WeaponShotGun : WeaponBehaviour
    {
        ...
        [Tooltip("Amount of bullets with a shot")]
        [SerializeField]
        private int pelletAmounts = 10;
        ...
        public override void Fire(float spreadMultiplier = 1.0f)
        {
          ...
          //  animator.Play(stateName, 0, 0.0f);
            ...
            StartCoroutine(ShotgunFire(muzzleSocket,rotation));
        ...
        private IEnumerator ShotgunFire(Transform muzzleSocket, Quaternion rotation)
        {
            ...
            for (int i = 1; i <= pelletAmounts; i++)
            {
                ...
                projectile = Instantiate(prefabProjectile, muzzleSocket.position,  rotation * Quaternion.Euler(degree * Mathf.Cos(2*Mathf.PI*i/pelletAmounts + randNum), degree * Mathf.Sin(2*Mathf.PI*i/pelletAmounts + randNum),0));   ///updown , rightleft, forwardback
            ...
            for (int i = 1; i <= pelletAmounts/2; i++)
            {
                ...
                float degree = theta * 180f / Mathf.PI;
                projectile = Instantiate(prefabProjectile, muzzleSocket.position, rotation * Quaternion.Euler(degree * Mathf.Cos(2 * Mathf.PI * i / (pelletAmounts/2) + randNum), degree * Mathf.Sin(2 * Mathf.PI * i / (pelletAmounts/2) + randNum), 0));   ///updown , rightleft, forwardback
                projectile.GetComponent<Rigidbody>().velocity = projectile.transform.forward * projectileImpulse;
                ApplyConditionOverload (projectile);
            }
            for (int i = 1; i <= pelletAmounts / 10; i++)
            {
                ...
                float degree = theta * 180f / Mathf.PI;
                projectile = Instantiate(prefabProjectile, muzzleSocket.position, rotation * Quaternion.Euler(degree * Mathf.Cos(2 * Mathf.PI * i / (pelletAmounts / 10) + randNum), degree * Mathf.Sin(2 * Mathf.PI * i / (pelletAmounts / 10) + randNum), 0));   ///updown , rightleft, forwardback
                projectile.GetComponent<Rigidbody>().velocity = projectile.transform.forward * projectileImpulse;
                ApplyConditionOverload(projectile);
            }
